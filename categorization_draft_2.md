## Draft 1
*(Breakout boards are listed in the least significant category BEFORE bare chips with the same function.)*

### Components
#### Resistors
*In least significant category, resistors are sorted first by value, then by power.*
- Single resistors
	- Through-hole resistors
		- Axial
			- General use
			- Precision & milspec
		- Radial
	- Panel-mount resistors
		- Ceramic resistors
		- Wire-wound resistors
		- Power resistors, other
	- SMT Resistors
		- General use
		- Precision & milspec
- Resistor networks & arrays
	- Through-hole ressitor networks
	- Surface-mount resistor networks
- Potentiometers & trimmers
	- Potentiometers & trimmers, through-hole
	- Potentiometers & trimmers, solder lug
	- Potentiometers & trimmers, SMT
#### Capacitors
*In least significant category, resistors are sorted first by capacitance, then by voltage.*
- Ceramic capacitors
	- Through-hole
	- SMT
- Electrolytic & Polymer capacitors
	- Through-hole
	- SMT
- Specialized capacitors
	- Tantalum capacitors
		- Through-hole
		- SMT
	- Film capacitors
	- Exotic materials, other
	- Supercapacitors
- Capacitor Networks & Arrays
	- Through-hole
	- SMT
Capacitors, variable
	- Through-hole
	- SMT
#### Fuses, discreet circuit protection
*In least significant category, fuses and varistors are sorted first by package (size), then by rating.*
- Automotive fuses
- Cartridge fuses
- Thermal fuses
- Polymer fuses
	- PTC Resettable fuses
		- Through-hole
		- SMT
	- Other
- Varistors
	- Through-hole
	- SMT
- Fuse holders, through-hole
- Fuse holders, panel mount
- Fuse holders, SMT
#### Diodes
*In least significant category, diodes are sorted alphabetically and numerically by part number, grouping parts from the same series.*
- Signal & Switching diodes
	- Through-hole
	- SMT
- Rectifier diodes
	- Through-hole
	- SMT
- Schottky diodes
	- Through-hole
	- SMT
- Zener diodes
	- Through-hole
	- SMT
- RF diodes
	- Through-hole
	- SMT
- Bridge rectifiers
	- Through-hole
	- Panel-mount & spade connector
	- SMT
- Ideal diodes
	- Through-hole
	- SMT
- Diode networks & arrays, other
	- Through-hole
	- SMT
#### Transistors
*In least significant category, transistors are sorted alphabetically and numerically by part number, grouping parts from the same series.*
- BJT
	- Through-hole
	- SMT
- FETs
	- FETs
		- Through-hole
		- SMT
	- MOSFETs
		- Through-hole
		- SMT
	- JFETs
		- Through-hole
		- SMT
- IGBTs
	- Through-hole
	- SMT
- PUTs
	- Through-hole
	- SMT
- Darlington transistors
	- Discreet Darlington transistors
		- Through-hole
		- SMT
	- Darlington transistor arrays
		- Through-hole
		- SMT
#### Other discreet semiconductor
*In least significant category, other semiconductors are sorted alphabetically and numerically by part number, grouping parts from the same series.*
- Varicaps
- TRIACs
- Thyristors
#### Coils / Relays
*In least significant category, inductors are sorted first by value, then by power rating.*
- Inductors
	- THT
	- SMT
- Ferrites
	- Ferrite beads
		- THT
		- SMT
	- Ferrite toroids
	- Snap-on ferrites
- Filters and reactors
- Transformers
*In least significant category, transformers are sorted by power.*
	- Power transformers
		- Through-hole
		- SMT
	- Signal transformers
		- Audio transformers
			- Through-hole
			- SMT
		- Ethernet transformers
			- Through-hole
			- SMT
- Relays
*In least significant category, relays are sorted first by maximum power switching capacity (DC or AC), then by coil voltage.*
	- Electromagnetic relays
		- Through-hole
		- SMT
	- Solid-state relays
		- Through-hole
		- SMT
#### LEDs and Illumination
- Discreet LEDs
- LED Arrays
- LED Strips
#### Buttons and Switches
#### Encoders
*Encoders for motor control or robotics are listed in the Robotics category.*
- Human-interface encoders
	- Through-hole
	- SMT
#### Connectors / Cables

### Integrated Circuits, Interfaces
#### ADC/DAC
*In least significant category, ADCs and DACs are sorted alphabetically and numerically by part number, grouping parts from the same series.*
- ADCs
	- Through-hole
	- SMT
- DACs
	- Through-hole
	- SMT
- Digital potentiometers
	- Through-hole
	- SMT
#### IO Expanders, Multiplexers, Shift registers
*In least significant category, these parts are sorted alphabetically and numerically by part number, grouping parts from the same series.*
*7400 and 4000 series parts are in their own category.*
- IO Expanders
	- Through-hole
	- SMT
- Digital multiplexers
	- Through-hole
	- SMT
- Analog multiplexers
	- Through-hole
	- SMT
- Shift registers
	- Through-hole
	- SMT
- Counter-shift registers
	- Through-hole
	- SMT
#### Oscillators, Crystals, RTC
- Oscillator ICs
	- Digitally controlled
		- Through-hole
		- SMT
	- Voltage controlled
		- Through-hole
		- SMT
- Crystals
	- Bare crystals
		- Through-hole
		- SMT
	- Crystal oscillators
		- Through-hole
		- SMT
- RTCs
	- Through-hole
	- SMT
#### Op Amps, Comparators
- General op-amps
	- Through-hole
	- SMT
- OTA
	- Through-hole
	- SMT
- Comparators
	- Through-hole
	- SMT
#### Filter, signal processor ICs
- Audio filter ICs
	- Through-hole
	- SMT
- DSP ICs
	- Through-hole
	- SMT
- Other signal processing
	- Through-hole
	- SMT
#### Memory
- PSRAM
	- Through-hole
	- SMT
- Flash
	- Through-hole
	- SMT
- EEPROM
	- Through-hole
	- SMT
- Computer memory
	- SSDs
	- SD Cards
	- USB flash drives
	- SD Card Connectors/Adapters
#### Logic
- 7400 Series
	- Through-hole
	- SMT
- 4000 Series
	- Through-hole
	- SMT
#### Serial, Debug, Programming
- Logic level converters
	- Monodirectional
		- Through-hole
		- SMT
	- Bidirectional
		- Through-hole
		- SMT
- UART
	- UART transcievers
		- Through-hole
		- SMT
- CAN Bus
	- CAN bus transcievers
		- Through-hole
		- SMT
- I2C
	- I2C multiplexers
		- Through-hole
		- SMT
- USB
	- USB controllers
		- Through-hole
		- SMT
	- PD controllers
		- Through-hole
		- SMT
	- USB converters/interfaces
		- USB to GPIO, UART, etc.
			- Through-hole
			- SMT
		- USB to PWM
			- Through-hole
			- SMT
		- USB to HID, keyboard, etc.
			- Through-hole
			- SMT
- Programmers
	- JTAG
	- AVR
	- UPDI
#### Power ICs
- Linear regulators
	- LDO
		- Through-hole
		- SMT
	- Other
		- Through-hole
		- SMT
- Switching regulators, buck/boost converters
	- Buck converters
		- Through-hole
		- SMT
	- Boost converters
		- Through-hole
		- SMT
	- Other
		- Through-hole
		- SMT
- Battery management/charge controllers
	- Battery charger ICs
		- Through-hole
		- SMT
	- Fuel gauges
		- Through-hole
		- SMT
	- Other/combined battery management ICs
		- Through-hole
		- SMT
- Power distribution ICs
	- Through-hole
	- SMT
- Other power management
	- Through-hole
	- SMT

### Development Boards
- Microcontrollers
	- AVR
		- ATTiny
		- ATMega
	- PIC
	- ARM
		- SAMD
		- RP2040/235x
		- STM32
		- Other ARM
	- ESP
		- ESP8266
		- ESP32
			- ESP32 Classic
			- ESP32-S
- Single Board Computers
*(AI cameras and Pi cameras are categorized with other cameras)*
	- Raspberry Pi 
		- 0
		- 1
		- 2
		- 3
		- 4
		- 5
		- CM
			- CM boards
			- Accessories/Breakouts
		- HATs and accessories
	- BeagleBone
	- Other
- Machine Learning & AI
	- AI Accelerators/TPUs/GPUs
- FPGA

### IoT & Wireless
- Bluetooth
- Cellular / LTE
- LoRa
- Mesh Network
- Radio Frequency
- Sattelite Communication
- Software Defined Radio
- WiFi
- IOT & Wireless Antennas
- Wireless & IoT Kits

### GPS / GNSS
- (GPS category takes priority over other sensors/IMUs)
- GPS / GNSS Boards and Modules
- GPS / GNSS Antennas

### Sensors
- Biometric Sensors
- Capacitive Sensors
- Current Sensors
- Distance Sensors
- Environmental Sensors
- Flex / Force Sensors
- Imaging Sensors
- Infared Sensors
- Miscellaneous Sensors
- Movement Sensors
- NFC Sensors
- RFID Sensors
- Sensor Kits
- Sount
- Weight

### Audio
- Audio ADCs, DACs, CODECs
- Audio Players/Triggers
- Audio Amplifiers
- FM/AM Tuners
- MIDI Interfaces
- Other Audio ICs
- Microphones
- Speakers, Buzzers, Piezo Elements
- Audio Connectors
- Audio Cables

### Displays/Video
- Character LCDs
- Graphic LCDs
- Color LCDs
- Monochrome OLEDs
- Color OLEDs
- E-Ink
- Alphaneumeric Displays
- HDMI, NTSC, PAL Monitors
- Video ICs
	
### E-Textiles & Crafting
- (E-textile/Craft category takes priority over all others)
- E-textile Microcontrollers
- E-Textile Lights
- E-Textile Switches and Sensors
- E-textile Power
- E-textile Materials
- Craft Electronics/Kits

### Robotics, Actuators, RC
- Actuators
	- Motors
	- Solenoids
	- Pumps
- Motor Drivers
- Motor Control
- RC Parts
- Robotics hardware
